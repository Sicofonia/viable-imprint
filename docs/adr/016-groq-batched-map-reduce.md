...
**Status:** Reverted (2026-09-10), never implemented. Written as a follow-up to ADR 015 after Groq's Developer tier turned out to be closed to new signups; before implementation started, the decision was made to abandon the whole Groq integration (ADR 015 included) and return to Mistral alone rather than build around the free tier's constraints. Kept as a record of the design (the `MapReduceStrategy`/`SingleCallMapReduce`/`BatchedMapReduce` shape, the real per-task input-size data) in case a similar batching need comes up again with any future provider.
