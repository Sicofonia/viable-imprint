...
**Status:** Implemented. Decision 1 (the overflow guard) shipped first; Decision 2 (map-reduce synthesis) shipped as a follow-up, initially wired to the four tasks the book that surfaced this bug needed most urgently (`synopsis`, `story-map`, `one-pager`, `press-dossier`), then extended to two more (`trailer-storyboard`, `goodreads-profile`) once that same book turned out to need those too — see "Implementation notes" for all three passes, including three real defects found and fixed (`story-map`'s waypoint duplication, `goodreads-profile`'s dropped title brackets, `trailer-storyboard`'s duration-budget overflow). The remaining six `single_chunk` tasks (`newsletter`, `article-draft`, `s4 briefing`, `s4 content-strategy`, `s5 evaluate`, `s5 homeostat`) still fall back to Decision 1's hard failure if they ever exceed `max_chars` — no `reduce_prompt` has been authored for them yet, per Decision 2's task-by-task, build-when-needed approach.
...
### 2. Map-reduce synthesis for long inputs — built as a follow-up, wired to six tasks
...
Each task that wants this gets its own `reduce_prompt` file, following this project's standing one-prompt-per-task convention (ADR 001) rather than a shared generic reducer — `press-dossier`'s reconciliation needs (heading fidelity against a fixed template) are not `story-map`'s (waypoint-list merging and de-duplication across chunks) or `synopsis`'s (picking one 150-250 word synthesis out of several drafts). Six reduce prompts were written and wired up, in two passes — `prompts/s1d/{synopsis,story_map,one_pager,press_dossier,trailer_storyboard,goodreads_profile}_reduce_task.txt`, each with a committed English reference under `prompts/examples/s1d/` — matching, in the end, every deliverable the book that surfaced this bug actually needed (the first four covered its most urgent needs; `trailer-storyboard` and `goodreads-profile` were added once that same book turned out to need those too). `trailer-storyboard`'s reduce prompt is structurally different from the other five: it's a **curation** job, not a merge — concatenating every draft's scenes would blow past 60/90 real seconds of video, so the reduce prompt selects the best scenes across the whole book and rebuilds exactly two storyboards within the same fixed runtime budget the original task uses. The remaining six `single_chunk` tasks (`newsletter`, `article-draft`, both System 4 tasks, both remaining System 5 tasks) were left without a `reduce_prompt`: nothing has ever actually hit the guard for them, and per the "why built task-by-task" reasoning below, writing and testing a reduce prompt against a hypothetical failure is exactly the premature work this ADR avoids elsewhere. Any of them can gain one the same way, the day a real run actually needs it.
...
**Easier:**
- A future long book hits a clear, immediate error naming the exact task, the exact character counts, and this ADR — instead of a silently corrupted deliverable discovered by chance during manual review, as happened here — for any of the six tasks without a `reduce_prompt` yet.
- For the six tasks that do have one (`synopsis`, `story-map`, `one-pager`, `press-dossier`, `trailer-storyboard`, `goodreads-profile`), a long book now gets a genuinely coherent, single document instead of either silent corruption or a hard stop — confirmed against the real book that surfaced this bug, whose six previously-corrupted outputs are now regenerated and correct as a direct result of this and the follow-up pass's real-call testing.
...
**Harder / needs care:**
- The six tasks without a `reduce_prompt` (`newsletter`, `article-draft`, both System 4 tasks, both remaining System 5 tasks) still hard-fail on any input exceeding `max_chars` — a regression from today's (broken but silently "working") behavior into a hard block, for exactly the books where getting it right matters most, until each one gets its own reduce prompt built and tested the same way the first six were.
- Each `reduce_prompt` is new prompt surface needing its own real-call reliability testing (heading fidelity, no fabricated reconciliation, correct arithmetic where a task has numeric constraints) before it can be trusted — confirmed to be a real, not theoretical, cost across both passes: `story-map`'s waypoint duplication, `goodreads-profile`'s dropped title brackets, and `trailer-storyboard`'s duration-budget overflow were all first-real-test defects, each fixed by one round of prompt hardening — see Implementation notes.
...
Decision 2 (second follow-up pass — `trailer-storyboard` and `goodreads-profile`, once the same real book turned out to need those too):

- [x] Write `prompts/s1d/trailer_storyboard_reduce_task.txt` (a curation prompt, not a merge — selects the best scenes across all drafts and rebuilds exactly two storyboards within the original fixed 60s/90s runtime budget) and `prompts/s1d/goodreads_profile_reduce_task.txt`, each with a committed English reference under `prompts/examples/s1d/`
- [x] Wire `reduce_prompt:` into both tasks' entries in `systems/s1d/tasks.yaml`, alongside their existing `single_chunk: true`
- [x] End-to-end test against the real oversized brief: `goodreads-profile`'s first real test dropped the brackets from its literal `[TÍTULO PENDIENTE]` fallback text — fixed by separating the "choose a title" instruction from the "write this literal fallback, brackets included" instruction, which more nested/ambiguous bracket-in-bracket phrasing had blurred together; re-tested and confirmed fixed. `trailer-storyboard`'s first real test produced a 90-second storyboard whose scenes summed to 85 seconds against an 80-second budget (each individual scene honored the 8-10s rule, but nothing checked the total) — fixed by adding an explicit pre-submission arithmetic check (sum every scene's duration, compare against the exact budget, cut or shorten if over); re-tested and confirmed both storyboards now land within budget with room to spare.
- [x] Regenerate the affected book's two additionally-corrupted `s1d` outputs (`trailer-storyboard`, `goodreads-profile`) now that they can be produced correctly — done as a direct byproduct of the end-to-end tests above
- [ ] Not built in this pass, left for whenever a real run actually needs one (per Decision 2's task-by-task reasoning): `reduce_prompt` for `newsletter`, `article-draft`, `s4 briefing`, `s4 content-strategy`, `s5 evaluate`, `s5 homeostat`
...
## Implementation notes — Decision 2, second follow-up (2026-08-31)

The book that surfaced this whole bug turned out to need `trailer-storyboard` and `goodreads-profile` regenerated too (both had been confirmed corrupted during the original investigation — 8 scene tables instead of 2, and 5x-duplicated headings, respectively). Built and tested end-to-end against the same real, oversized brief:

- **`goodreads-profile`: a real defect found and fixed on the first real test.** The title heading came back as `# TÍTULO PENDIENTE` — missing the brackets required by the template's literal fallback text (`"[TÍTULO PENDIENTE]"`). The original reduce-prompt wording nested the "pick a title" instruction and the "write this exact fallback string" instruction inside one bracket-within-bracket sentence, which the model resolved by keeping the words but dropping the outer punctuation. Fixed by splitting the instruction into two explicit, separate clauses — one for choosing a consensus title across drafts, one for the literal fallback text with an explicit "brackets included, never write it without them" rule. Re-tested: not only did the brackets survive, the reduce call also picked a confident, concrete title (`A través del Asia Central: Diario de la expedición sueca (1933-1935)`) and, incidentally, a correctly-named, confident author biography (Sven Hedin) where the first run had also hedged into a vague, unnamed "the author" — both symptoms traced back to the same underlying instruction-clarity fix. One minor, non-blocking observation left as-is: the final "Estantes y géneros sugeridos" list came back with 12 tags against the template's stated 5-8 — a soft quantity guideline, not a structural defect, not worth a further iteration.
- **`trailer-storyboard`: a real arithmetic defect found and fixed on the first real test.** The reduce call correctly produced exactly two tables (60s and 90s) with sensible scene selection and correct per-scene 8-10s durations — but the 90-second table's scenes summed to 85 seconds against an 80-second budget (90s total, minus 5s intro, minus 5s outro), with the final scene running from 1:21 to 1:30 and directly overlapping the outro's reserved 1:25-1:30 slot. The 60-second table, by contrast, was exact (5 × 10s = 50s, matching its budget precisely) — the defect was specifically in aggregating a *new* combined timeline during reduction, something the original per-chunk task never had to do. Fixed by adding an explicit rule: sum every scene's duration per table before finalizing, compare against the exact budget (50s / 80s), and cut or shorten scenes if over — obeying each scene's individual 8-10s bound isn't sufficient on its own. Re-tested: both tables now land within budget (49s/50s and 75s/80s), with no scene overlapping the outro's reserved window.
- Both defects follow this project's established pattern once more (ADR 002 points 6-7, ADR 006 points 9-10, and this ADR's own `story-map` finding above): a new prompt shape's first real test finds something a careful read of the prompt wouldn't have caught, fixed by one round of targeted wording, no code involved. Two-for-two on real defects this pass is a higher hit rate than the first four reduce prompts (one-for-four) — plausibly because `goodreads-profile` and `trailer-storyboard` are structurally the two most different reduce jobs from the "merge N complete documents" shape the first four share (one is unusually bracket-nested, the other is a curation-under-a-fixed-numeric-budget problem, not a merge at all), so the established merge-oriented instructions transferred less cleanly.
