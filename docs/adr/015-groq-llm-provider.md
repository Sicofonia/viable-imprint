...
Groq's free tier for `openai/gpt-oss-120b` is bounded on four dimensions at once — requests per minute, requests per day, tokens per minute, tokens per day — and the first ceiling reached returns `429` (or, for a single request whose own size exceeds the ceiling, `413` — see Decision 4). Per Groq's own rate-limit documentation the free-plan ranges are RPM 10–30, RPD 100–14.4K, **TPM 1.2K–15K**, TPD 3.6K–500K; secondary sources place `openai/gpt-oss-120b` specifically at roughly 30 RPM / 1,000 RPD / **8,000 TPM** — and this account's own TPM figure is now directly confirmed at exactly **8,000**, read straight from a real 413 response body ("Limit 8000, Requested ...") during implementation testing, not just inferred from secondary sources. RPM/RPD/TPD remain unconfirmed for this account. In any case it's the *order of magnitude* that breaks things:
...
- **TPM meters prompt and completion tokens together over the rolling minute, but — corrected against real testing, see Decision 3's own note — a single request's *admission* looks mainly at that request's own prompt size against the limit, not prompt+output combined.** Completion tokens land in the account's used-total only once a call finishes, competing with *later* requests' headroom rather than inflating the current one's admission check. This still leaves a real margin worth keeping, just for a different reason than originally assumed here: in a multi-chunk task, each chunk's completion eats into the same rolling window the next chunk's prompt has to fit inside, which is exactly the throttling pattern the next bullet describes.
- **The chunked System 1B tasks fare better but not well.** `cleanup`/`ortho`/`copyedit` use `llm_text`'s default `max_chars: 8000` — ~2,700 input tokens plus a ~2,300-character system prompt, call it ~5,500–6,000 tokens per round trip. That fits inside 8,000 TPM, but only about one chunk per minute — confirmed for real during implementation testing: a 3-chunk `ortho`/`copyedit` run routinely needed one or two waits of 5–30s between chunks as each one's completion ate into the next one's headroom. The book currently in production (`books/historia-expedicion-asia-vol3`, 901,457 characters at the `ortho`/`copyedit` stage) is **113 chunks**, or roughly 600,000 tokens for a single pass — around two hours of pure rate-limit waiting, and far beyond any plausible tokens-per-day allowance.
...
**Corrected against real testing** — the recommended starting value of `4000` is deliberately half of the confirmed 8,000 TPM ceiling, but not for the reason originally guessed here (that TPM charges a single request's input and output together against its own admission). A real oversized request measured ~34,000 tokens by this check's own `~3 chars/token` estimate but only ~21,000 by Groq's real tokenizer — roughly 1.6× over. The margin exists mainly to absorb that estimation slop, so this *local* check doesn't reject a request Groq's real tokenizer would have accepted; it secondarily covers the multi-chunk throttling pattern the Context section's "constraint that is not a drop-in" describes. It should still be re-derived from the account's actual dashboard figures if they differ from the 8,000 confirmed here.
...
**As implemented, corrected against a real call:** the case this section describes — a single request's own token count exceeding the ceiling — turned out not to come back as a 429 at all. A real oversized request (preflight disabled) returned **HTTP 413** ("Request too large for model `openai/gpt-oss-120b`... on tokens per minute (TPM): Limit 8000, Requested 21151, please reduce your message size and try again."), which isn't in the retryable-status set to begin with, so it already fails fast via the base class's ordinary non-retryable-error handling — no `_is_fatal_rate_limit()` involvement needed for that path. `_is_fatal_rate_limit()`'s "Limit/Requested" parsing is kept as a defensive fallback for a 429 that happens to carry the same fatal-looking arithmetic, but real 429s observed during testing were all the genuinely transient kind ("Limit 8000, Used 7655, Requested 3549" — Requested alone admissible, correctly retried, correctly succeeded once the window cleared). One small addition made because of this: a new `_extra_error_hint()` hook, called on any non-retryable error, that on a 413 specifically points back at `llm.max_request_tokens` — the mechanism that's supposed to catch this before it ever reaches Groq. See Implementation notes below.
...
- [x] Create `providers/llm/openai_chat.py` with `OpenAIChatProvider(LLMProvider)`: payload construction, the retry/backoff loop moved verbatim (budget, backoff shape, `httpx.TransportError` handling, original-exception re-raise), `usage` accumulation, `_error_detail()`, and the hooks `API_URL` / `DISPLAY_NAME` / `RATE_LIMIT_HINT` / `_extra_payload()` / `_log_rate_limit_headroom()` / `_is_fatal_rate_limit()` (plus `_validate_content()` and `_extra_error_hint()`, added during implementation — see Implementation notes)
- [x] Reduce `providers/llm/mistral.py` to a subclass, keeping its existing rate-limit hint and header-echo logic as overrides, and preserving its explanatory comments (they record two real incidents' findings)
- [x] Verify the extraction with a real Mistral call — `pipeline.py s1b cleanup books/test/s1b/source/zayagan-chp1.txt` (9,829 chars, one chunk) — confirming identical behavior, not just an identical-looking diff.

**Groq provider:**

- [x] Create `providers/llm/groq.py`: endpoint, hint text, `reasoning_effort` + `include_reasoning: false` in `_extra_payload()`, Groq's two-dimensional header echo (reset values echoed as duration strings, not parsed as floats), `_is_fatal_rate_limit()` per Decision 4, and the empty-`content` guard
- [x] Add the `groq` branch to `get_llm_provider()` with a `GROQ_API_KEY` check mirroring the Mistral one; update the "Unknown LLM provider" message to list both
- [x] Remove both `"mistral-medium-latest"` fallbacks ([`providers/__init__.py:20`](../../providers/__init__.py#L20), [`engines/llm_text.py:133`](../../engines/llm_text.py#L133)); raise a clear `ClickException` naming `llm.model` when it is absent
- [x] Add `GROQ_API_KEY` to `.env.example`

**Preflight:**

- [x] Add `_estimate_tokens()` and the `llm.max_request_tokens` check to `engines/llm_text.py` — over the chunks before the completion loop, and again over the joined drafts before the reduce call
- [x] Confirm the check is a no-op when `llm.max_request_tokens` is absent

**Config and docs:**

- [x] Update `config.example.yaml`: `groq` defaults, `reasoning_effort`, `max_request_tokens` (with the "half your TPM, because TPM counts output too" reasoning), and Groq's paid rates in `pricing:` with the shadow-cost caveat spelled out
- [x] Update `config.yaml` to `provider: groq` / `model: openai/gpt-oss-120b`
- [x] Update the README: setup section (`GROQ_API_KEY`, `llm.model` now required), the provider-abstraction paragraph, and a note on the free-tier ceiling versus `max_chars: 100000` tasks
- [x] Confirm the account's real limits — TPM confirmed directly (see Implementation notes); RPM/RPD/TPD not independently confirmed (no real call in this pass happened to hit those dimensions)
...
- [x] `s1b cleanup` on `books/test/s1b/source/zayagan-chp1.txt` — first real Groq call. This task runs on the raw **English** source, before translation, so it's checking OCR-cleanup fidelity and the three existing normalizers (`_DOUBLED_HEADING_RE`, `_CODE_FENCE_RE`, `_BRACKETED_HEADING_RE`), not Spanish quality.
- [x] `s1b ortho` and `s1b copyedit`, each run directly against the **existing** `books/test/s1b/translated/es/zayagan-chp1.txt` — not against a fresh `translate` run. These are the two Spanish-language, fidelity-critical tasks (`temperature: 0.0` against Groq's 0.5–0.7 recommendation is most likely to show here) and don't require `translate` to have just run, only for its prior output to already be on disk, which it is.
- [x] One `single_chunk`-shaped task against a real oversized input, confirming the preflight fires immediately with the numbers named and no tokens spent
- [x] One deliberately oversized single request with the preflight disabled — confirmed the ADR's *assumption* about which status code fires was wrong (413, not a fatal-shaped 429), fixed, re-confirmed. See Implementation notes.
- [x] Confirm `s3 dashboard` reports Groq token usage and shadow cost correctly, and that a book with rows from both providers reads sensibly

---

## Implementation notes (2026-09-09)

Built and tested end-to-end against real invocations on both providers, not simulated:

- **Extraction confirmed byte-for-byte behavior-preserving.** `pipeline.py s1b cleanup books/test/s1b/source/zayagan-chp1.txt` against the real Mistral account produced the exact same retry/backoff sequence and final failure message the pre-refactor code would have (`"Mistral request failed: 429 after 14 attempt(s) over 542s — Rate limit exceeded. Mistral ties rate limits to account tier..."`) — the account is still genuinely rate-limited (14 attempts, giving up at the 600s budget), which is itself useful confirmation: the retry loop behaved identically to before under a real, sustained 429 condition, not just a quick synthetic check.

- **First real Groq call, `s1b cleanup`: clean.** OCR page-break artifacts (running headers like "2 RUMOURS OF A KHAN", bare page numbers, a stray embedded image filename) were correctly removed, text otherwise faithful, no doubled headings, no code fences, no bracket-wrapped headings — none of the three existing Mistral-era normalizers fired, and none needed to.

- **A real, new defect found on `s1b ortho`'s first real Groq call, fixed, and re-verified.** The Spanish output wrapped bare Roman numerals in brackets mid-sentence — `CAPÍTULO [I]` instead of `CAPÍTULO I`, `el siglo [VI]` instead of `el siglo VI` — a pattern none of the three existing normalizers catch (they only strip brackets around a *whole* `# [heading]` line). Same underlying tendency as the existing `_BRACKETED_HEADING_RE` fix (something structurally significant gets wrapped in brackets unprompted), just landing on a bare numeral instead of a full heading. Fixed with a fourth normalizer, `_strip_bracketed_roman_numerals()`: matches `\[([IVXLCDM]+)\]` (uppercase-only, so it can't collide with this project's real markup — `[i]`, `[sc]`, `[FN: ...]` are all lowercase or colon-bearing), validated against the strict Roman-numeral form before stripping, to keep the false-positive surface small (a bracketed acronym that happens to also parse as a valid numeral, e.g. `[CD]`, is the accepted residual risk — same class of tradeoff the other three normalizers already accept). Re-tested against both real captured instances plus the project's actual `[i]`/`[sc]`/`[FN: ...]` markup (unit test) and against a fresh real `ortho` call (end-to-end): both original defects gone, all real markup untouched.

- **`s1b copyedit`: clean**, including several real transient `429`s along the way (see below), all correctly retried and resolved — no stray brackets, output reads as a faithful copyedit pass.

- **A real, corrected assumption in Decision 4.** The ADR's own text assumed a single request whose token count alone exceeds the ceiling would come back as a `429` with a parseable "Limit/Used/Requested" body. A real oversized request (preflight disabled, largest `single_chunk`-shaped input) instead came back as **HTTP 413**: `"Request too large for model \`openai/gpt-oss-120b\`... on tokens per minute (TPM): Limit 8000, Requested 21151, please reduce your message size and try again."` — no `Used` field at all, and a different status code than assumed. This is arguably a *cleaner* design on Groq's part than what was assumed: 413 (structurally inadmissible) and 429 (transiently throttled, always worth retrying) turn out to be genuinely distinct signals, not one status code serving both meanings. Since 413 isn't in `_RETRYABLE_STATUS`, it already failed fast via the base class's ordinary non-retryable-error handling — the elaborate `_is_fatal_rate_limit()` regex this section originally described for exactly this case never actually fires for it. Fixed in three small ways rather than left inconsistent with reality: (1) the regex was loosened to not require a `Used` field, so it still matches if a fatal-shaped `429` ever does occur; (2) `_is_fatal_rate_limit()`'s docstring and the module-level comment now say plainly that this was empirically found to be a `413`, not the case the hook was designed for; (3) a new `_extra_error_hint()` hook (default no-op, called on any non-retryable error) was added, with Groq's override pointing a `413` back at `llm.max_request_tokens` — confirmed live: the real error message now ends with `"...This pipeline's own llm.max_request_tokens preflight (config.yaml) is meant to catch this before spending a request on it — check it's set, or lower max_chars for this task."`.

- **The preflight (Decision 3) fires correctly and instantly.** Tested against a real oversized `single_chunk`-shaped input with `llm.max_request_tokens: 4000` set (the real `config.yaml` default): failed in 0.6s, naming the exact estimated token count and the remedy, zero API calls made. With the same input and `llm.max_request_tokens` omitted (preflight disabled), the request reached Groq for real and produced the 413 described above — confirming both the enabled and no-op paths.

- **Real transient throttling observed and handled correctly, repeatedly, across `ortho` and `copyedit`.** `_log_rate_limit_headroom()`'s low-headroom echo fired correctly with real duration-string reset values (e.g. `"Rate limit headroom low: 109 of 8000 tokens/minute remaining, resets in 59.182s."`); several real `429`s with genuinely transient shape (`Requested` alone under `Limit`, e.g. `"Limit 8000, Used 7655, Requested 3549"`) were correctly retried via the `retry-after`-driven backoff and succeeded once the window cleared — confirming the fatal/transient distinction holds up in practice, not just in the synthetic cases checked before any real call was made. The 8,000 TPM ceiling binds often at `max_chars: 8000` (System 1B's default): a 3-chunk task routinely needs one or two waits of 5–30s to complete, consistent with the ADR's own "roughly one chunk per minute" estimate for this task shape.

- **`s3 dashboard` confirmed correct for a mixed-provider book.** After the real Groq runs above, `books/test`'s dashboard showed `s1b.cleanup`/`s1b.ortho`/`s1b.copyedit` as `groq / openai/gpt-oss-120b` with nonzero shadow cost (e.g. `$0.002610` for `cleanup`, computed from Groq's paid rates despite running on the free tier, per Decision 2), alongside a pre-existing `s1d.press-dossier` row still showing `mistral / mistral-medium-latest` — correctly marked `(stale)`, since re-running `cleanup` cascade-invalidated it per ADR 009. This is the mixed-provenance consequence the ADR names, observed for real rather than just reasoned about, and the dashboard read sensibly: separate provider/model per row, a combined total cost and duration across both.

- **Not exercised in this pass, unchanged from the ADR's own stated scope:** none of the twelve `max_chars: 100000` / `single_chunk` tasks were run against Groq's free tier for real (only a synthetic oversized fixture, used to trip the preflight/413 paths) — they're expected to fail exactly as documented, and confirming that for all twelve wasn't needed to validate the mechanism itself.

- **A second look at `max_request_tokens: 4000`'s own rationale, prompted by a direct question after the first review pass, corrected the same way as Decision 4 above.** The value was never wrong, but the *reason* given for it — "half of TPM, because TPM counts a single request's input and output together" — didn't hold up against the same real 413 body already captured: Groq's `Requested` figure lined up with prompt-tokens-alone (via Groq's real tokenizer), not prompt+output, and every successful multi-chunk run showed completion tokens landing in the account's used-total only *after* a call finished, not reserved against that call's own admission. The real reason to keep the margin is this check's own estimate running conservative relative to Groq's tokenizer (the same 413's request measured ~34K tokens by this project's `~3 chars/token` heuristic against Groq's own ~21K) — setting it to the full 8,000 risks this *local* check rejecting a request Groq would actually have accepted. Corrected in `config.yaml`, `config.example.yaml`, and this ADR's Context/Decision 3 text; the configured value (`4000`) did not change, only why it's there. Also worth noting: with the project's current task shapes, this distinction is not yet load-bearing — every task sits at `max_chars: 8000` (comfortably under either 4,000 or 8,000 estimated tokens) or `max_chars: 100000` (over both), so no existing task's behavior depends on exactly where between 4,000 and 8,000 this is set. It matters only as a safety margin for a future task with an in-between `max_chars`.
