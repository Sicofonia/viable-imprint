...
**A task marked `single_chunk: true` guards against silent chunk-overflow (ADR 014).** Every task above from `s1d synopsis` through `s1d goodreads-profile`, plus `s1d article-draft`, `s4 briefing`, `s4 content-strategy`, `s5 evaluate`, and `s5 homeostat`, must synthesize its entire input (a brief, a scan digest, a candidate description) in a single LLM call — none of them are designed to process their input chunk-by-chunk the way `s1b cleanup`/`ortho`/`copyedit` or `s1d brief` are. If a particular book or run ever produces an input long enough to exceed that task's `max_chars` (100,000 characters, by convention), the task now fails immediately with a clear error naming the task, the character counts, and the chunk count actually produced — rather than silently running once per chunk and concatenating several complete, mutually inconsistent documents into one file. This is a fail-safe, not a fix for long inputs: a task that hits this needs either a shorter input or a map-reduce synthesis step (specified but not yet built — see the ADR).
