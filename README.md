...
This project is LLM-agnostic and translation-provider-agnostic by design. The provider abstraction (`providers/llm/` and `providers/translation/`) means that swapping providers requires adding one file and one line in `config.yaml` — no changes to the pipeline steps themselves. **Groq and Mistral both ship today** (ADR 015): both speak the same OpenAI-compatible chat-completions wire format, so `providers/llm/openai_chat.py` holds the shared retry/backoff/diagnostics logic and each provider is a ~60-line subclass supplying its endpoint and what's genuinely different (rate-limit header shapes, provider-specific request fields). A provider with a different wire format entirely (e.g. Anthropic's Messages API) would implement `providers/llm/base.py`'s `LLMProvider` interface directly instead of subclassing `openai_chat.py`. Distribution-platform-agnostic too: `providers/sales/` (ADR 011) auto-detects which platform a sales report came from rather than assuming any one imprint's own (e.g. IngramSpark), so adding a platform this project doesn't ship an adapter for yet is the same one-file shape, no changes to `engines/sales_ingest.py`.
...
**`engines/`** — the generic execution logic, shared across every system: `llm_text` (chunk input, call an LLM with a given prompt file — reused by thirteen different tasks across three systems, each just pointing it at a different prompt; a task can set `single_chunk: true` when it must synthesize its entire input in one call rather than being processed chunk-by-chunk — `llm_text` then fails loudly, before making any LLM call, if its input ever turns out to need more than one chunk, instead of silently running the task once per chunk and concatenating several complete, mutually inconsistent documents together, unless the task also sets `reduce_prompt` to a second, task-specific prompt file: when set, a multi-chunk input still runs the normal per-chunk pass, then makes one extra reconciliation call — the N per-chunk drafts, each wrapped in a `<borrador numero="i" total="N">` tag, reduced by `reduce_prompt` into one final coherent document — instead of failing. See `docs/adr/014-system1d-brief-chunk-overflow-guard.md`. Also enforces an optional `llm.max_request_tokens` ceiling from `config.yaml` (ADR 015) — before any API call, refuses to send a request estimated over that size, naming the task and the numbers, instead of relying on a provider's own rate-limit retry to eventually fail on a request that could never have succeeded; unset by default, meaningful mainly for a provider with a hard per-minute token ceiling like Groq's free tier.), `translation` (chunk input, call a translation provider), `odt_format` (render markup to `.odt`), `metadata_doc` (assemble a document from bibliographic facts with no LLM call at all), `feed_scan` (pull new items from a curated watchlist of external sources — RSS/Atom feeds, Project Gutenberg's bulk catalog, and an imprint's own catalog page — no LLM call), `homeostat_scan` (gather System 3/4/decision-log data into one file, no LLM call), `homeostat_render` (render the final self-contained dashboard HTML, no LLM call), `newsletter_scan` (gather this month's production activity, notes, and non-repetition tracking lists into one file, no LLM call), `newsletter_track` (check/record the featured explorer and dish, write the final clean copy, no LLM call), `sales_ingest` (parse a manually-downloaded royalty CSV via a pluggable `providers/sales/` format adapter, dedup-append matching revenue to the book's manifest, no LLM call). Engines are written once and reused by any task that needs that shape of work.
...
**`pipeline.py s3`** is System 3 (Performance Monitoring) — `dashboard` is hand-written, read-only. Every task run (manual or via `s2 run`) now records its duration, and — for the two engines that call an external provider (`llm_text`, `translation`) — token/character usage, provider, model, and an optional cost in USD, right alongside its `s2` ledger entry. `pipeline.py s3 dashboard` shows a portfolio-wide summary across every book; `pipeline.py s3 dashboard <book>` breaks one book down task by task. Cost is opt-in: it's only computed when you've filled in an optional `pricing:` block in `config.yaml` (illustrative placeholders ship in `config.example.yaml` — verify against your actual plan before trusting the numbers); duration and raw usage are captured either way. This is deliberately scoped to *reporting* only — no resource allocation, budget tracking, or pricing decisions, which stay a human call. See `docs/adr/005-system-3-performance-monitoring.md`.

**`s3 dashboard` also flags portfolio outliers (ADR 010)** — Cyberstride-style peer-comparison, not a budget check (this project has no plan/budget figure to compare against): a book whose API cost is `N`× the rest of the portfolio's average gets flagged in the portfolio view; a specific task whose duration is `N`× that same task's average across every other book gets flagged in the per-book view. Both configurable and both optional via an `s3.deviation` block in `config.yaml` (`cost_multiplier`, `duration_multiplier`), same pattern as `pricing:` — omit it and the dashboard is unchanged. Needs at least 3 books (or 3 completions of a task) before it flags anything at all; below that, a peer average is too noisy to mean much.
...
**A task marked `single_chunk: true` guards against silent chunk-overflow (ADR 014).** Every task above from `s1d synopsis` through `s1d goodreads-profile`, plus `s1d article-draft`, `s4 briefing`, `s4 content-strategy`, `s5 evaluate`, and `s5 homeostat`, must synthesize its entire input (a brief, a scan digest, a candidate description) in a single LLM call — none of them are designed to process their input chunk-by-chunk the way `s1b cleanup`/`ortho`/`copyedit` or `s1d brief` are. If a particular book or run ever produces an input long enough to exceed that task's `max_chars` (100,000 characters, by convention), the task fails immediately with a clear error naming the task, the character counts, and the chunk count actually produced — rather than silently running once per chunk and concatenating several complete, mutually inconsistent documents into one file. Six of these tasks — `s1d synopsis`, `s1d story-map`, `s1d one-pager`, `s1d press-dossier`, `s1d trailer-storyboard`, `s1d goodreads-profile` — instead reconcile a multi-chunk input into one coherent document via a second, task-specific `reduce_prompt` rather than failing (`trailer-storyboard`'s is a curation prompt, not a merge, since its output has a fixed 60/90-second runtime budget that concatenating every chunk's scenes would blow past); the remaining six still hard-fail until a `reduce_prompt` is written for them, built only once a real run actually needs one rather than speculatively. See the ADR for which tasks have one today.
...
| `s3 sales-ingest` | System 3 | A manually-downloaded royalty CSV (any file path — `pipeline.py s3 sales-ingest <book_slug> <csv_file>`) | Receipt in `s3/sales-ingest/<date>/` and a deduped append to the book's `manifest.yaml` `sales:` block — no LLM call |
...
- `format_styles.yaml` — `format` doesn't build a document from scratch; it loads your own `.odt` template (page setup, margins, and named paragraph styles already defined) and appends the manuscript to it, using your template's own style names. This file maps a small set of structural roles — `chapter_number`, `chapter_title`, `first_paragraph`, `body` — to whatever you've actually named those styles in your template. Chapter structure is detected directly from the text (a short, all-uppercase line containing a roman numeral or digit is the chapter number; an immediately following short uppercase line is the title) — no markup tags required. Inline `[sc]` markup is left as plain visible text in the output by design; applying that formatting is a manual step. `[i]` and `[FN: ...]` are the two exceptions — `format` converts `[i]...[/i]` into a real italic character span, and `[FN: ...]` into a real ODF footnote (a proper reference mark plus a separately-rendered note body, not visible bracket text in the flow), sitting flush against the word it follows and numbered sequentially across the whole document. Footnote handling copes with both closing styles `cleanup` actually produces in practice (a bare `]`, or the prompt's own specified `[/FN]]`) and won't be tricked by other markup (e.g. an italicized book title) inside a footnote's own text — a footnote's own `[i]` markup is converted too, inside its separately-rendered body. Note that whether a given `[i]` span *should* be italicized is still entirely `cleanup`'s call — `format` converts whatever markup it's given, it doesn't second-guess it (e.g. a foreign term's own quoted translation shouldn't be italicized, but that's a prompt-wording fix, not something safe to pattern-match away here). **Publishing in more than one physical format** (different trim sizes, paperback vs. hardcover)? Name each `.odt` template after the format it belongs to (`6x9template.odt`, `hardcover-a5.odt`, ...) and use `format`'s `--template` option to pick a different one per run — you don't need a separate `format_styles.yaml` for each, since the named style *roles* are usually identical across your own templates, only the file differs: `pipeline.py s1b format <file> --template templates/hardcover-a5.odt`.
...
- An API key for the LLM provider you configure — Groq (default) or Mistral (the pipeline calls both directly over HTTP — no SDK required for either, see ADR 015)
- A DeepL API key
...
Open `.env` and add the API key for whichever LLM provider you selected in `config.yaml` (`llm.provider: groq` or `mistral`), plus DeepL's:
...
`llm.model` is required regardless of provider — there's no cross-provider default, since a Mistral model name sent to Groq (or vice versa) would just fail. See `config.example.yaml`'s comments for both providers' settings, including Groq-specific keys (`reasoning_effort`, `max_request_tokens`) that Mistral ignores. See `docs/adr/015-groq-llm-provider.md` for the full reasoning, including a real limitation worth knowing before you rely on Groq's free tier: several of this pipeline's tasks (System 1D's expansion tasks, both System 4 tasks, both System 5 tasks — anything reading a whole brief/briefing/candidate in one call) send requests too large for Groq's free-tier tokens-per-minute ceiling and cannot run there today; `llm.max_request_tokens` makes that failure immediate and clear rather than a confusing multi-minute retry, but doesn't remove the ceiling itself. Those tasks currently need Mistral, a different provider, or a paid Groq tier.
...
To ingest sales/royalty data (ADR 011), no config is needed — `pipeline.py s3 sales-ingest` auto-detects IngramSpark's and KDP's report formats from the CSV's own header row. If you sell through a platform this project doesn't recognize yet, or a platform changes its export layout, add a new small adapter under `providers/sales/` (one file, mirroring `providers/sales/ingramspark.py`) rather than waiting on this project to add it for you.
...
With an `s3.deviation` block configured (see Setup), an outlier gets an inline `!` and a trailing `Flagged: ...` line explaining the comparison — a book whose cost is `N`× the rest of the portfolio's average in the portfolio view, a task whose duration is `N`× that same task's average elsewhere in the per-book view. Peer-comparison only, not a budget check — see `docs/adr/010-system-3-deviation-flagging.md`.

**Record what a title actually earned:**

```bash
uv run python pipeline.py book set-isbn life-as-explorer 978-1-0686007-2-2   # once, whenever the ISBN is assigned
uv run python pipeline.py s3 sales-ingest life-as-explorer downloads/ingramspark-q2-2026.csv
```

Drop in whatever royalty CSV you downloaded from IngramSpark, KDP, or (once you've added a `providers/sales/` adapter for it) any other platform you sell through — the format is auto-detected from the CSV's own header row, no need to say which platform it's from. Re-ingesting the same file twice is a safe no-op: matching rows are deduped against what's already recorded, not double-counted. `s3 dashboard` then shows revenue (grouped by currency — no automatic conversion) and a *reported* margin, explicitly labeled as revenue minus tracked API cost only, not `vsm.md`'s full title-economics metric. No portal automation — this is manual download, drop-in the CSV, same as every other "no background process" step in this pipeline. See `docs/adr/011-system-3-sales-ingestion.md`.
