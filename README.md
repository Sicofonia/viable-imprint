...
**`pipeline.py s1c`** is System 1C (Design and Graphic Production) — print-file preparation for a PDF you've already exported from LibreOffice by hand, after reviewing the formatted `.odt` that `s1b format` produced (the "1B → 1C handoff", above). 1C has no `systems/s1c/tasks.yaml` and no engine: like `s3 dashboard` and `s2`'s own commands, it's hand-written, not manifest-driven, makes no LLM call, and writes nothing to the System 2 ledger — deliberately absent from the task table below, the same way `s2 status`/`s2 run` and `s3 dashboard` are. `pipeline.py s1c check <pdf> [--trim-w --trim-h]` inspects a PDF's `MediaBox`/`TrimBox`/`BleedBox` page boxes in mm, optionally checked against an expected trim size (LibreOffice's PDF export only ever writes `MediaBox`, never the other two, which print vendors' prepress checks expect); `pipeline.py s1c set-interior <in> <out> --trim-w --trim-h` writes an explicit, no-bleed `TrimBox` for the interior PDF; `pipeline.py s1c set-cover <in> <out> --trim-w --trim-h --bleed [--spine-w]` writes a centered `TrimBox` plus a full-page `BleedBox` for the cover PDF, with an optional vendor-calculated spine width folded into the trim width. The underlying logic lives in `pdf_bleed_tool.py` at the repo root (also runnable standalone, e.g. `python pdf_bleed_tool.py check cover.pdf`, for quick debugging outside `pipeline.py`) — added to meet Bibliomanager's PDF technical requirements, which differ from IngramSpark's. This is the last step before manually uploading to a distribution platform (System 1D's job per `docs/vsm.md`, itself not automated here). See `docs/adr/019-system1c-pdf-print-prep-tooling.md`.
